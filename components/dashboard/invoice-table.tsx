...
export function InvoiceTable({
  ...
  const fallbackName =
    authUser?.firstName || authUser?.lastName
      ...
      : authUser?.client?.primaryContactName || "Valued Client";
  const defaultClientName = clientName || fallbackName;
  const defaultClientNumber = clientNumber || authUser?.client?.clientNumber;
  const defaultClientEmail = clientEmail || authUser?.email;
  const defaultPlanName = planName || authUser?.client?.selectedPlan;
  ...
  const getCleanDescription = (inv: InvoiceItem) => {
    const raw = inv.description || inv.planName || defaultPlanName || "Service Plan";
    return raw.replace(/\s*(Membership Statement|Manual Invoice)\s*$/i, "").trim();
  };
  ...
  const handleDownloadPdf = async (inv: InvoiceItem) => {
    ...
    try {
      ...
      generateInvoicePdf({
        ...
        clientName: inv.clientName || defaultClientName,
        clientNumber: inv.clientNumber || defaultClientNumber,
        clientEmail: inv.clientEmail || defaultClientEmail,
        planName: inv.planName ? inv.planName.replace(/\s*(Membership Statement|Manual Invoice)\s*$/i, "").trim() : cleanDesc,
        description: cleanDesc,
        amount: inv.amount,
        billingFrequency: inv.billingFrequency || billingFrequency,
        paymentMethod:
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-[#D9E4EC]/60 text-sm font-medium text-[#243746]">
                {paginatedInvoices.map((inv) => {
                  const desc = getCleanDescription(inv);
                  return (
                    <tr key={inv.id} className="hover:bg-[#F7FAFC] transition-colors">
                      ...
                      <td className="py-4 px-4 text-[#64748B]">{desc}</td>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-[#243746] text-sm">{desc}</span>
